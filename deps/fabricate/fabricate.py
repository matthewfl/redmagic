...
def relative_to_abs_paths(args):
    """ Attempt to convert relative paths to abs paths. """
    ret = []
    for a in args:
        lex = shlex.split(a)
        s = []
        for l in lex:
            if os.path.exists(l) or os.path.exists(os.path.dirname(l)):
                s.append(os.path.abspath(l))
            else:
                s.append(l)
        ret.append(' '.join(s))
    return ret
